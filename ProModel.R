...
##Constants
h<-0.656879285 #Frequency of previous 
i<-0.685595319 #Proline frequency + previous 
...
Pro<-0 #Pro counter in each peptide
ProC<-c() #vector of Pro number per peptide
...
Ns <- 0 #Simulated peptide number is reset to 0
  ...
  repeat {
    ...
    if (r>h & r<=i) AA="Pro" else AA="Other"  
    #determination of the amino acid based on the result of r
    if (r>h & r<=i) Pro<-Pro+1 
    #annotation of Pro number per peptide
    G[l]=AA #the new AA is added to the growing peptide
    if (l==lmax) break #when lmax is reached the loop stops
  }
  ProC[Ns]<-Pro #the amount of Pro per peptide is saved
  Pro<-0
  #Pro counter is set to zero for the next peptide
  if (ProC[Ns]/Lpep[Ns]==1 & Lpep[Ns]==4) print (G) #print successful peptide for visual checking
  if (ProC[Ns]/Lpep[Ns]==1 & Lpep[Ns]==4) print (NS) #print nº of simulation to check the progress
  G<-c() #the AAs vector is emptied into each peptide
  l<-0 #length is restored
  if (ProC[Ns]/Lpep[Ns]==1 & Lpep[Ns]==4) break 
  #If a Pro(4) is generated, the loop stops
  }
...
#Mode of the probability of the appearance of Pro(4)
par(mar = c(5, 6, 4, 2)) 
hist(1/Nse,col="blue", breaks=800, xlim=c(0,mean(1/Nse)), main = "Pro(4)", 
...
#Histogram of the probability of Pro(4) appearing
